...
export default class Emitter {
	...
	_setBufferUpdateRanges( keys ) {
		...
		for ( const i = this.attributeCount - 1; i >= 0; --i ) {
	...
	_assignPositionValue( index ) {
		const prop = this.position,
			...
			value = prop._value,
			spread = prop._spread,
			positionDistribution = prop._distribution;
	...
	_assignForceValue( index, attrName ) {
		const prop = this[ attrName ],
			value = prop._value,
			spread = prop._spread,
			forceDistribution = prop._distribution;
		...
		if ( attrName === 'acceleration' ) {
			const drag = clamp( randomFloat( this.drag._value, this.drag._spread ), 0, 1 );
	...
	_assignAbsLifetimeValue( index, propName ) {
		...
		if ( arrayValuesAreEqual( prop._value ) && arrayValuesAreEqual( prop._spread ) ) {
			const value = Math.abs( randomFloat( prop._value[ 0 ], prop._spread[ 0 ] ) );
	...
	_assignAngleValue( index ) {
		...
		if ( arrayValuesAreEqual( prop._value ) && arrayValuesAreEqual( prop._spread ) ) {
			const value = randomFloat( prop._value[ 0 ], prop._spread[ 0 ] );
			array.setVec4Components( index, value, value, value, value );
		}
		else {
			array.setVec4Components( index,
	...
	_resetBufferRanges() {
		...
		for ( let i = this.bufferUpdateCount - 1; i >= 0; --i ) {
			const key = keys[ i ];
	...
	_checkParticleAges( start, end, params, dt ) {
		for ( let i = end - 1; i >= start; --i ) {
			...
			// Increment age
			let age = params[ index + 1 ];
			const maxAge = params[ index + 2 ];
	...
	_activateParticles( activationStart, activationEnd, params, dtPerParticle ) {
		...
		for ( let i = activationStart; i < activationEnd; ++i ) {
			const index = i * 4;
			...
			const dtValue = dtPerParticle * ( i - activationStart );
